...
async function initFirebaseStorage() {
    ...
    if (typeof global !== "undefined" && global._vy_firebase_admin_sdk) {
        ...
        const { getStorage, ref, uploadString, getDownloadURL } =
            await import('./index.esm-Dccu7Uct.js');
...
class ApiKeysData {
    ...
    async create(keyData) {
        return await database.set(COLLECTION$4, keyData);
    }

    async update(id, updates) {
        return await database.update(COLLECTION$4, id, updates);
    }
    ...
    async getByHash(keyHash) {
        const results = await database.query(COLLECTION$4, { keyHash });
...
const COLLECTION$3 = "organizations";
...
class OrganizationsData {
    ...
    async getPersonalOrgByUser(userId) {
        const orgs = await database.query(COLLECTION$3, {
            uid: userId,
            isPersonal: true,
        });
    ...
    async create(name, ownerUserId, orgData) {
        ...
        const id = await database.set(COLLECTION$3, org);
        return id;
    }
    ...
    async update(id, updates) {
        ...
        return await database.update(COLLECTION$3, id, filteredUpdates);
    }
    ...
    async ensureUniqueToken(name) {
        ...
        let token = baseToken;
        while (true) {
            const existingOrgs = await database.query(COLLECTION$3, {
                token: token,
            });
    ...
    async ensurePersonalOrg(user) {
        ...
        try {
            if (poid) {
                existing = await database.get(COLLECTION$3, poid);
            } else {
    ...
    async addMember(orgId, userId) {
        return await database.update(COLLECTION$3, orgId, {
    ...
    async createInvite(orgId) {
        ...
        await database.update(COLLECTION$3, orgId, {
    ...
    async acceptInvite(token, userId, role = "member") {
        ...
        await database.update(COLLECTION$3, org.id, {
...
//# sourceMappingURL=index-BHzDQ1RV.js.map
