...
async function initFirebaseStorage() {
    ...
    if (typeof global !== "undefined" && global._vy_firebase_admin_sdk) {
        ...
        const { getStorage, ref, uploadString, getDownloadURL } =
            await import('./index.esm-CwE2BKsk.js');
...
v1App.get("/fetch/:storage/*path", async (req, res) => {
    const { storage, path } = req.params;
    const storageMap = {
        1: "firebase",
        2: "minio",
        3: "seaweed",
    ...
    try {
        ...
        const url = await storageInstance.createSignedUrl(path, "GET", 15 * 60);
...
v1App.get("/video/results/:id", async (req, res) => {
    ...
    try {
        ...
        const chunkResult = chunks.map((chunk) => ({
            ...
            expressionsUrl: `/fetch/${storageMap[chunk.storage] || chunk.storage}/${chunk.expressionsPath}`,
            demographicsUrl: `/fetch/${storageMap[chunk.storage] || chunk.storage}/${chunk.demographicsPath}`,
        }));
        ...
        const summaryUrl = await storage.createSignedUrl(
            summaryPath,
            "GET",
            15 * 60
        );
...
//# sourceMappingURL=index-C0A3Q_yI.js.map
