...
async function initFirebaseStorage() {
    ...
    if (typeof global !== "undefined" && global._vy_firebase_admin_sdk) {
        ...
        const { getStorage, ref, uploadString, getDownloadURL } =
            await import('./index.esm-BqfBgPp2.js');
    ...
    watchJobStatus(jobId, onChange) {
        return new Promise((resolve, reject) => {
            ...
            const notify = () => {
                ...
                onChange(keys.map((key) => status[key]));
            };

            const watchJob = (id, depth) => {
                if (watched[id]) return;
                watched[id] = true;

                const indent = "  ".repeat(depth);

                const unsub = database.watch("jobs", id, (jobData) => {
                    status[id] =
                        `${indent}${jobData.type} - ${jobData.status} ${jobData.message}`;

                    if (jobData.children && jobData.children.length > 0) {
                        for (const childId of jobData.children) {
                            watchJob(childId, depth + 1);
                        }
                    }
...
//# sourceMappingURL=index-DTKcRXh2.js.map
