...
The development environment is hosted within docker using Nginx and `server.js`. The production environment is hosted on Firebase and uses Firebase Functions.
...
## Changes to endpoints regex

The firebase function endpoints mapped by regular expressions in `firebase.json` need to be replicated to `conf/nginx.conf`. The docker container needs to be rebuilt and restarted for those changes to take effect.
