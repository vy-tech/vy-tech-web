...
import { S as Score, b as progress, a as activeBoxManager } from './annotations-UH0kPXsT.js';
import { e as eventBus } from './eventbus-BMI3jhi1.js';
import { H as Hierarchy, e as eventsData } from './hierarchy-DwL93XLR.js';
...
class Storage {
    ...
    async createMultipartUpload(_remotePath, _contentType = null) {
        throw new Error("createMultipartUpload() must be implemented by subclass");
    }

    async createSignedPartUrl(_remotePath, _uploadId, _partNumber, _expiresIn = 3600) {
        throw new Error("createSignedPartUrl() must be implemented by subclass");
    }
    ...
    async requestPartUrl(uploadId, path, partNumber, orgId = null, storageType = "seaweed") {
        const headers = await this.getAuthHeaders();

        const resp = await fetch("/api/file/upload/part", {
            method: "POST",
            headers,
            body: JSON.stringify({ uploadId, path, partNumber, oid: orgId, storage: storageType }),
        });
...
class S3Storage extends Storage {
    ...
    async createSignedPartUrl(remotePath, uploadId, partNumber, expiresIn = 3600) {
        await this._ensureClient();
...
//# sourceMappingURL=summarizer-BqRtjQod.js.map
