...
class EventsData {
    ...
    async getByHierarchy(hierarchy) {
        console.log("Fetching event by hierarchy:", hierarchy);
        if (this.hierarchy == null) return null;

        const events = await database.query("events", { hierarchy: hierarchy });

        if (events && events.length > 0) {
            this.current = events[0];
            console.log("Loaded event", this.current);
            return this.current;
        }
...
//# sourceMappingURL=events-DFZZlzll.js.map
