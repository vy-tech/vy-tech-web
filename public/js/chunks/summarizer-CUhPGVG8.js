...
class Storage {
    static getInstance(config, secrets, storageType = "firebase") {
        if (storageType === "s3") return new S3Storage({ config, secrets });
        if (storageType === "minio") return new MinioStorage({ config, secrets });
        if (storageType === "seaweed") return new SeaweedStorage({ config, secrets });
        if (storageType === "firebase") return new FirebaseStorage({ config, secrets });
        if (storageType === "local") return new FileSystemStorage({ config, secrets });
...
class FileSystemStorage extends Storage {
    ...
    async _downloadFileOnce(remotePath, localPath, progressCallback, fs) {
        ...
        console.log(
            `Copied ${this.storageType}:${remotePath} to ${localPath}`
        );
    }
    ...
    async _uploadFileOnce(localPath, remotePath, progressCallback, fs) {
        ...
        console.log(
            `Copied ${localPath} to ${this.storageType}:${remotePath}`
        );
    }
}
...
//# sourceMappingURL=summarizer-CUhPGVG8.js.map
