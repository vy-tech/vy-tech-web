...
function assertSecrets(secrets, section, className) {
    if (
        ...
        !secrets.get(section, "access_key")
    ) {
        throw new Error(
            `${className} requires "${section}" credentials — not available in this context`
        );
    }
}
...
class S3Storage extends Storage {
    ...
    constructor(parent = {}) {
        ...
        this.bucketName = this.config
            ? this.config.get(section, "bucket_name", null)
            : null;
...
class FileSystemStorage extends Storage {
    constructor(parent = {}) {
        ...
        this.basePath = this.config
            ? this.config.get("local", "base_path", "data")
            : "data";
    }
    ...
    async deleteFile(remotePath) {
        ...
        if (!fs.existsSync(filePath)) {
            this.log.warning(
                `File ${remotePath} not found in FileSystemStorage`
            );
            return;
        }
...
//# sourceMappingURL=summarizer-D5kEH5N2.js.map
