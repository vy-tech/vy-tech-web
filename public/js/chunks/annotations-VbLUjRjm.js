...
import { d as database } from './db-DCDBtw2W.js';
...
class Score {
    ...
    async loadWindowFromSchedule(scheduleIndex) {
        if (scheduleIndex < this.loadSchedule.length) {
            ...
            await this.loadDemographics(this.getDemographicsUrl(chunk));
        }
    }
    ...
    async loadDemographics(url) {
        ...
        for (const entry of demographics) {
            result[entry.time] = result[entry.time] || {};
            result[entry.time][entry.detection] = entry.count;
        }

        this.currentDemographics = result.values();
...
//# sourceMappingURL=annotations-VbLUjRjm.js.map
