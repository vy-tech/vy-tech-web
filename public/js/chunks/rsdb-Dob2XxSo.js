...
import { a as app } from './rsfirebase-DS4AlBEm.js';
import { L as Logger, _ as _registerComponent, C as Component, r as registerVersion, a as _isFirebaseServerApp, F as FirebaseError, n as LogLevel, g as getApp, c as _getProvider, d as getDefaultEmulatorHostnameAndPort, i as isCloudWorkstation, p as pingServer, u as updateEmulatorBanner, t as deepEqual, e as createMockUserToken, b as getModularInstance, A as isSafari, l as getUA, S as SDK_VERSION } from './index.esm2017-5fAidh0d.js';
...
def pushid(self, now=None):
        push_chars = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ_abcdefghijklmnopqrstuvwxyz"

        # Date of original proof of concept video capture
        epoch = int(datetime(2024, 9, 14, 3, 27, 0).timestamp() * 1000)
        now = now or int(time.time() * 1000)  # Current time in ms

        # Encode timestamp into 8 characters
        time_stamp_chars = [''] * 8
        timestamp = now - epoch
        for i in reversed(range(8)):
            time_stamp_chars[i] = push_chars[timestamp % len(push_chars)]
            timestamp //= len(push_chars)
        if timestamp != 0:
            raise ValueError("Timestamp didn't fully convert")

        last_rand_chars = [random.randint(0, len(push_chars)-1) for _ in range(12)]
        rand_chars = [push_chars[i] for i in last_rand_chars]

        return ''.join(time_stamp_chars + rand_chars)
*/

function pushid(now = null) {
    const pushChars =
...
export { Form as F, List as L, getFirestore as a, getDocs as b, collection as c, doc as d, firestore as f, getDoc as g, limit as l, orderBy as o, pushid as p, query as q, setDoc as s, where as w };
//# sourceMappingURL=rsdb-Dob2XxSo.js.map
